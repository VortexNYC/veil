...
func TestPostgresRecoveryAfterKEKLoss(t *testing.T) {
	...
	// Recovery: open the wrap, re-seed org_keys under the new KEK.
	master, err = fresh.OpenRecoveryWrap(ctx, "org", owner, recoveryKey)
	if err != nil {
		t.Fatal(err)
	}
	if err := fresh.ReseedOrgKey(ctx, "org", master); err != nil {
		t.Fatal(err)
	}
